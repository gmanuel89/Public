...
# Before we start
Tk().withdraw()
tkMessageBox.showinfo(title="MGF converter", message="This application will replace each Cmpd value with the correspondent MSMS value in the selected MGF file")

# Message for selection (open)
Tk().withdraw()
tkMessageBox.showinfo(title="MGF selection", message="Select the MGF file to be converted")

# MGF file selection
Tk().withdraw()
inputfile = tkFileDialog.askopenfilename(filetypes=[('MGF files','.mgf')])

# Message for selection (save)
Tk().withdraw()
tkMessageBox.showinfo(title="MGF selection", message="Select where to save the converted MGF file")

# Where to save the CSV file
Tk().withdraw()
outputfile = tkFileDialog.asksaveasfilename (defaultextension='.mgf', filetypes=[('MGF files','.mgf')])
...
# Conversion done!
Tk().withdraw()
tkMessageBox.showinfo(title="MGF conversion done", message="The conversion has been successfully performed!")
