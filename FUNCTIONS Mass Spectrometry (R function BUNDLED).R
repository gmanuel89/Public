...
functions_mass_spectrometry <- function() {
  ...
  ################## FUNCTIONS - MASS SPECTROMETRY 2017.21.12.1 ################
    ...
    if (method == "getURL") {
      try({
        ...
        if ("RCurl" %in% installed.packages()[,1]) {
          ...
          try({
            library("tcltk")
            tkmessageBox(title = "Select mirror for downloading packages", message = "Please, select the mirror from which to download the package files required", icon = "info")
          }, silent=TRUE)
          cat("\nPlease, select the mirror from which to download the package files required\n")
          install.packages("RCurl", repos = NULL, quiet = TRUE, verbose = FALSE, lib = Sys.getenv("R_LIBS_USER"), destdir = Sys.getenv("R_LIBS_USER"))
          library(RCurl)
        }
      }, silent = TRUE)
      there_is_internet <- FALSE
...
#  ##################################################### INSTALL REQUIRED PACKAGES (FROM SPECIFIED PACKAGE SET (zip files))
...
#		install_and_load_required_packages(required_packages = required_packages, repository = NULL, update_packages = update_packages, print_messages = print_messages)
#	}
#	
#	# Try locally
...
#  }
  ...
  # This function installs and loads the selected packages
  install_and_load_required_packages <<- function(required_packages, repository = NULL, update_packages = FALSE, print_messages = FALSE) {
    ### Check internet connection
