...
preprocessing_window_function <- function() {
	...
	smoothing_choice <- function() {
		...
		if (smoothing == "YES" || smoothing == "") {
			...
			# Strength
			smoothing_strength <- select.list(c("small","medium","strong","stronger"), title="Choose")
	...
	baseline_subtraction_choice <- function() {
		...
		if (baseline_subtraction == "YES" || baseline_subtraction == "") {
			...
			if (baseline_subtraction_algorithm == "SNIP") {
				baseline_subtraction_iterations <- tclvalue(baseline_subtraction_iterations)
	...
	normalization_choice <- function() {
		...
		if (normalization == "YES" || normalization == "") {
			...
			if (normalization_algorithm == "TIC") {
				normalization_mass_range <- tclvalue(normalization_mass_range)
				normalization_mass_range_value <- as.character(normalization_mass_range)
				if (normalization_mass_range != 0) {
	...
	commit_preprocessing_function <- function() {
		...
		# Baseline subtraction iterations
		baseline_subtraction_iterations <- tclvalue(baseline_subtraction_iterations)
		baseline_subtraction_iterations_value <- as.character(baseline_subtraction_iterations)
		baseline_subtraction_iterations <- as.integer(baseline_subtraction_iterations)
		# Escape the function
		...
		.GlobalEnv$normalization_mass_range <- normalization_mass_range
		.GlobalEnv$normalization_mass_range_value <- normalization_mass_range_value
		.GlobalEnv$baseline_subtraction_iterations <- baseline_subtraction_iterations
		.GlobalEnv$baseline_subtraction_iterations_value <- baseline_subtraction_iterations_value
	...
	baseline_subtraction_iterations <- tclVar("")
	normalization_mass_range <- tclVar("")
	##### Window
	...
	mass_range_entry <- tkentry(preproc_window, width=15, textvariable=mass_range)
	...
	preprocess_spectra_in_packages_of_entry <- tkentry(preproc_window, width=10, textvariable=preprocess_spectra_in_packages_of)
	...
	baseline_subtraction_iterations_entry <- tkentry(preproc_window, width=15, textvariable=baseline_subtraction_iterations)
	tkinsert(baseline_subtraction_iterations_entry, "end", "200")
	# Normalization
	normalization_button <- tkbutton(preproc_window, text="Normalization", command=normalization_choice)
	normalization_mass_range_entry <- tkentry(preproc_window, width=15, textvariable=normalization_mass_range)
	tkinsert(normalization_mass_range_entry, "end", "0")
	# Commit preprocessing
